...
public class VNCScreenEvents
{
    ...
    //
    /**
     * translate the from the RFBClient received KeyEvent and send it to the fireKeyEvent method. At the time key is the primaryLevelUnicode
     * key, and the KeyEvent needs the raw code TODO add or use the list from KeyEvent.class to extend the vk-list in keysym.java
     */
    public void translateKeyEvent(RFBClient client, boolean down, int key)
    {
        ...
        if (down)
        {
            ...
            if ((key >= 0x0020 & key <= 0x007E) | ((key >= 0x00A0) & (key <= 0x00FF)))//  || ( virtualKey == KeyEvent.VK_BACK_SPACE ) )
            {
