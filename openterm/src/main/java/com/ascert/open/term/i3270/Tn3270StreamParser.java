...
public class Tn3270StreamParser implements TnStreamParser
{
 ...
 /*                p 4.3 table 4-1                            */
     ...
     * The process of sending a write type command and performing that command is called a <i>write</i> operation. Five write commands are
     ...
     * <TR><TD>1</TD><TD>WCC reset bit. When set to 1, it resets partition characteristics to their system-defined defaults. When set to 0,
     * the current characteristics remain unchanged (no reset operations are performed).</TD></TR>
     * <TR><TD>2, 3 & 4</TD><TD>Printer Operations N/A</TD></TR>
     * <TR><TD>5</TD><TD>Sound alarm bit. When set to 1, it sounds the audible alarm at the end of the operation if that device has an
     * audible alarm.</TD></TR>
     * <TR><TD>6</TD><TD>Keyboard Restore Bit. When set to 1, this bit unlocks the keyboard. it also resets the AID byte.</TD></TR>
     * <TR><TD>7</TD>
     * <TD>Bit 7 resets MDT bits in the field attributes. When set to 1, all MDT bits in the device's existing character buffer are reset
     * before any data is written or orders are performed.</TD></TR></TABLE>
     *
     */
     ...
     * <UL>
     * <LI>Sets the implicit partition size to the default size, if in implicit state</LI>
     * <LI>Resets a Program Check Indication, if one exists.</LI>
     * <LI>Erases the character buffer by writing null characters into all buffer locations.</LI>
     * <LI>Sets all the associated character attributes and extended field attributes to their default value(X'00).</LI>
     * <LI>Erases all field validation attributes.</LI>
     * <LI>Sets the current cursor position to 0. If directed to a partition, autoscroll is performed, if necessary, to position the window
     * at offset (0, 0).</LI>
     * <LI>If bit 1 of the WCC is set to 1, EW does the following:  </LI>
     * <UL>
     * <LI>Resets the inbound reply mode to Field.</LI>
     * <LI>Resets to implicit partition state, if currently in explicit partitioned state. It destroys all partitions, creates implicit
     * partition 0 with default screen size, and sets inboud PID to 0 and INOP to Read Modified.</LI>
     * </UL>
     * <LI>Provides an acknoledgment of any outstanding read or enter if the keyboard restore bit in the WCC is set to 1.</LI>
     ...
     * <UL>
     * <LI>Clears all the unprotected character locations of the partition to nulls and sets any character attributes affected to their
     * default values.</LI>
     ...
     * <LI>Repositions the cursor to the first character location, after the field attribute, in the first unprotected field of the
     ...
     * <i>3270 Data Stream Programmer's Reference</i>. Suffice it to say That we're telling the host:
     ...
     * The process of sending data inbound is called a <i>read operation</i>. A read operation can be initiated by the following:
     * <UL>
     * <LI>The host application sending an explicit read command</LI>
     * <LI>The host application program sending a Read Partition structured field specifying Read Buffer, Read Modified, or Read Modified
     * All.</LI>
     ...
     * of the read data stream the AID, and cursor address are known as the <i>read heading</i>. The inbound data stream format is as
     * follows:
     * <P>
     * <TABLE border = 1>
     ...
     * <TR><TD>7</TD><TD>MDT identifies modified fields during Read Modified Command operations.<BR>
     ...
     * <code>((counter1 & 0x3F) << 6) + (counter2 & 0x3F)</code>
     */
     ...
     * <li><b>If the current buffer address is not a field attribute, the MF order should be rejected</b></li>
     * </ul>
     */
